...
function buildBuildCommandsSection(stack) {
    ...
    if (!cmds || Object.keys(cmds).filter(k => cmds[k]).length === 0)
        return '';
    const lines = [];
...
function buildPersonaDirective(stack) {
    ...
    if (fw)
        return `Act as a Senior ${fw} developer with deep expertise in ${stack.primaryLanguage.name} and the full ${fw} ecosystem.`;
...
export function generateInstructions(stack, outputDir, options) {
    const base = readTemplate('base-instructions.md');
    if (!base)
        throw new Error('Base instructions template not found');
