...
export function readAiOsFile(relPath) {
    try {
...
export function getMemoryGuidelines() {
...
export function getRepoMemory(query, category, limit) {
    ...
    const filtered = entries
        .filter((entry) => {
        if (c && entry.category.toLowerCase() !== c)
            return false;
        if (!q)
            return true;
...
export function rememberRepoFact(title, content, category, tags) {
...
export function searchFiles(query, filePattern, caseSensitive = false) {
    try {
...
export function buildFileTree(dir, depth = 0, maxDepth = 4) {
    if (depth > maxDepth)
        return [];
...
export function getTrpcProcedures() {
...
export function getApiRoutes(filter) {
    ...
    function scanNextApiDir(dir, prefix = '') {
        try {
            ...
            for (const entry of entries) {
                ...
                const methods = ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].filter((m) => new RegExp(`export\\s+(?:async\\s+)?function\\s+${m}`).test(content));
                if (methods.length === 0)
                    continue;
...
export function getFileSummary(filePath) {
...
export function getImpactOfChange(filePath) {
...
export function getDependencyChain(filePath) {
    const newGraphPath = path.join(ROOT, '.github', 'ai-os', 'context', 'dependency-graph.json');
    const legacyGraphPath = path.join(ROOT, '.ai-os', 'context', 'dependency-graph.json');
    const graphPath = fs.existsSync(newGraphPath) ? newGraphPath : legacyGraphPath;
    if (!fs.existsSync(graphPath)) {
...
export function checkForUpdates() {
    ...
    try {
        const toolPkg = JSON.parse(fs.readFileSync(path.join(__dirname, '..', '..', 'package.json'), 'utf-8'));
    ...
    const updateAvailable = cMaj > iMaj ||
        (cMaj === iMaj && cMin > iMin) ||
