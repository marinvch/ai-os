...
export function suggestImprovements(): string {
  ...
  if (!hasEnvExample) {
    suggestions.push(
      '**Missing `.env.example`**: Document required environment variables so `get_env_vars` can surface them.',
    );
  }
  ...
  if (!fs.existsSync(path.join(ROOT, '.github', 'COPILOT_CONTEXT.md'))) {
    suggestions.push(
      '**Missing `COPILOT_CONTEXT.md`**: Re-run the AI OS installer (`npx -y github:marinvch/ai-os --refresh-existing`) to generate the session context card for better session continuity.',
    );
  }
  ...
  if (!fs.existsSync(path.join(ROOT, '.github', 'ai-os', 'recommendations.md'))) {
    suggestions.push(
      '**Missing `recommendations.md`**: Re-run the AI OS installer (`npx -y github:marinvch/ai-os --refresh-existing`) to generate stack-specific tool recommendations.',
    );
  }
  ...
  if (!fs.existsSync(memoryPath)) {
    suggestions.push(
      '**No repository memory found**: Use `remember_repo_fact` to capture key architectural decisions.',
    );
  } else {
    const content = fs.readFileSync(memoryPath, 'utf-8').trim();
    if (!content) {
      suggestions.push(
        '**Empty repository memory**: Use `remember_repo_fact` to capture key architectural decisions and conventions.',
      );
    }
  }
  ...
  if (!fs.existsSync(archPath)) {
    suggestions.push(
      '**Missing architecture doc**: Re-run the AI OS installer (`npx -y github:marinvch/ai-os --refresh-existing`) to rebuild `.github/ai-os/context/architecture.md`.',
    );
  }
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (!config.persistentRules || config.persistentRules.length === 0) {
        suggestions.push(
          '**No persistent rules defined**: Add `persistentRules` in `.github/ai-os/config.json` for rules that survive context window resets (e.g. "use shared components from components/ui").',
        );
      }
      if (config.recommendations === false) {
        suggestions.push(
          '**Recommendations disabled**: Set `"recommendations": true` in `.github/ai-os/config.json` to enable stack-specific tool suggestions.',
        );
      }
    } catch {
      // ignore
  ...
  return ['## Improvement Suggestions', '', ...suggestions.map((s) => `- ${s}`)].join('\n');
}
