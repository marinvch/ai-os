...
export function readAiOsConfig(outputDir) {
...
function detectExistingAiContext(rootDir) {
    ...
    if (exists(rootDir, '.github/instructions'))
        add('.github/instructions/', 'instructions');
    ...
    if (!exists(rootDir, '.github/ai-os/context/stack.md') && exists(rootDir, '.ai-os/context/stack.md'))
        add('.ai-os/context/stack.md (legacy)', 'docs');
    if (!exists(rootDir, '.github/ai-os/context/architecture.md') && exists(rootDir, '.ai-os/context/architecture.md'))
        add('.ai-os/context/architecture.md (legacy)', 'docs');
    if (!exists(rootDir, '.github/ai-os/context/conventions.md') && exists(rootDir, '.ai-os/context/conventions.md'))
        add('.ai-os/context/conventions.md (legacy)', 'docs');
    if (exists(rootDir, 'docs/ai/session_memory.md'))
        add('docs/ai/session_memory.md', 'docs');
    if (exists(rootDir, 'AGENTS.md'))
        add('AGENTS.md', 'other');
    if (exists(rootDir, 'CLAUDE.md'))
        add('CLAUDE.md', 'other');
    if (exists(rootDir, '.cursor/rules'))
        add('.cursor/rules/', 'other');
    if (exists(rootDir, '.windsurfrules'))
        add('.windsurfrules', 'other');
...
function generateStackDoc(stack) {
    ...
    if (stack.patterns.bundler)
        lines.push(`- **Bundler:** ${stack.patterns.bundler}`);
    if (stack.patterns.linter)
        lines.push(`- **Linter:** ${stack.patterns.linter}`);
    if (stack.patterns.formatter)
        lines.push(`- **Formatter:** ${stack.patterns.formatter}`);
    if (stack.patterns.testFramework)
        lines.push(`- **Test Framework:** ${stack.patterns.testFramework}`);
    if (stack.patterns.ciCdProvider)
        lines.push(`- **CI/CD:** ${stack.patterns.ciCdProvider}`);
...
function mergeSections(existing, updated) {
    ...
    for (const [heading, content] of existingSections) {
        if (heading !== '__preamble__' && !updatedSections.has(heading))
            result.push(content);
    }
    return result.join('\n');
}
/** Returns absolute paths of all managed files. */
export function generateContextDocs(stack, outputDir) {
    ...
    const managed = [];
    const track = (p) => { managed.push(p); return p; };
    ...
    if (!fs.existsSync(memoryReadmePath)) {
        writeIfChanged(memoryReadmePath, [
    ...
    if (!fs.existsSync(memoryFilePath)) {
        ...
        fs.writeFileSync(memoryFilePath, preambleEntries.map(e => JSON.stringify(e)).join('\n') + '\n', 'utf-8');
    }
