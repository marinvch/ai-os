...
import { DEPENDENCY_RECOMMENDATIONS, FRAMEWORK_RECOMMENDATIONS, LANGUAGE_RECOMMENDATIONS, UNIVERSAL_RECOMMENDATIONS, } from './registry.js';
export function collectRecommendations(stack) {
...
export function getSkillsGapReport(stack, skillsLockPath) {
    const collected = collectRecommendations(stack);
    const recommendedSkills = new Set(collected.skills.map(s => s.name));
    let installed = [];
    try {
        const lock = JSON.parse(fs.readFileSync(skillsLockPath, 'utf-8'));
...
export function generateRecommendations(stack, outputDir) {
    const collected = collectRecommendations(stack);
...
export function buildRecommendationsText(stack) {
    const collected = collectRecommendations(stack);
