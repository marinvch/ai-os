...
function pruneLegacyArtifacts(targetDir, options) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const ext2 = path.extname(entry.name).toLowerCase();
      if (!MANAGED_EXTENSIONS.has(ext2)) continue;
      try {
...
function applyFallbacks(content, fallbacks = {}) {
  return content.replace(PLACEHOLDER_RE, (match2) => {
    return fallbacks[match2] ?? "";
  });
}
...
function detectDrift(cwd) {
  ...
  const infos = [];
  const healthy = [];
  for (const { path: path30, description } of REQUIRED_FILES) {
    if (!existsSync(join(cwd, path30))) {
      errors.push({ path: path30, kind: "missing", severity: "error", message: `${description} is missing`, fix: FIX_CMD });
    } else {
      healthy.push(path30);
    }
  }
...
function detectLanguages(rootDir) {
  ...
  for (const file of files) {
    const ext2 = path12.extname(file).slice(1).toLowerCase();
    if (!ext2) continue;
    const lang = EXTENSION_MAP[ext2];
    if (!lang) continue;
    if (!counts[lang]) counts[lang] = { count: 0, extensions: /* @__PURE__ */ new Set() };
    counts[lang].count++;
    counts[lang].extensions.add(ext2);
  }
...
function detectFromPython(rootDir) {
  ...
  const content = files.map((f) => readFile(path13.join(rootDir, f))).join("\n").toLowerCase();
  if (!content) return [];
  const frameworks = [];
...
function detectFromGo(rootDir) {
  const goMod = readFile(path13.join(rootDir, "go.mod"));
  if (!goMod) return [];
  const frameworks = [];
...
function detectFromRust(rootDir) {
  const cargo = readFile(path13.join(rootDir, "Cargo.toml"));
  if (!cargo) return [];
  const frameworks = [];
...
function detectFromJava(rootDir) {
  const pomXml = readFile(path13.join(rootDir, "pom.xml"));
  const buildGradle = readFile(path13.join(rootDir, "build.gradle")) + readFile(path13.join(rootDir, "build.gradle.kts"));
...
function detectFromDotnet(rootDir) {
  ...
  const csprojContent = csproj ? readFile(path13.join(rootDir, csproj)).toLowerCase() : "";
...
function detectFromRuby(rootDir) {
  const gemfile = readFile(path13.join(rootDir, "Gemfile")).toLowerCase();
  if (!gemfile) return [];
...
function detectFromBun(rootDir) {
  if (!fs10.existsSync(path13.join(rootDir, "bun.lockb"))) return [];
  return [{ name: "Bun", category: "backend", template: "bun" }];
}
function detectFromDeno(rootDir) {
  const hasDenoJson = fs10.existsSync(path13.join(rootDir, "deno.json")) || fs10.existsSync(path13.join(rootDir, "deno.jsonc"));
  if (!hasDenoJson) return [];
  return [{ name: "Deno", category: "backend", template: "deno" }];
}
function detectFromPhp(rootDir) {
  const hasWpConfig = fs10.existsSync(path13.join(rootDir, "wp-config.php"));
  const hasWpContent = fs10.existsSync(path13.join(rootDir, "wp-content"));
  const hasWpIncludes = fs10.existsSync(path13.join(rootDir, "wp-includes"));
  const indexPhpPath = path13.join(rootDir, "index.php");
  ...
  const composer = readJson(path13.join(rootDir, "composer.json"));
  if (!composer) return [];
...
function detectBun(rootDir) {
  if (fs10.existsSync(path13.join(rootDir, "bun.lockb"))) {
    return [{ name: "Bun", category: "backend", template: "bun" }];
  }
  const pkg = readJson(path13.join(rootDir, "package.json"));
...
function detectDeno(rootDir) {
  ...
  if (denoFiles.some((f) => fs10.existsSync(path13.join(rootDir, f)))) {
    return [{ name: "Deno", category: "backend", template: "deno" }];
  }
  return [];
}
...
function detectPackageManager(rootDir) {
  if (exists(path14.join(rootDir, "bun.lockb"))) return "bun";
  if (exists(path14.join(rootDir, "pnpm-lock.yaml"))) return "pnpm";
  if (exists(path14.join(rootDir, "yarn.lock"))) return "yarn";
  if (exists(path14.join(rootDir, "package-lock.json"))) return "npm";
  if (exists(path14.join(rootDir, "Cargo.lock")) || exists(path14.join(rootDir, "Cargo.toml"))) return "cargo";
  if (exists(path14.join(rootDir, "go.sum")) || exists(path14.join(rootDir, "go.mod"))) return "go";
  if (exists(path14.join(rootDir, "Pipfile.lock"))) return "pip";
  if (exists(path14.join(rootDir, "poetry.lock"))) return "poetry";
  if (exists(path14.join(rootDir, "pom.xml"))) return "maven";
  if (exists(path14.join(rootDir, "build.gradle")) || exists(path14.join(rootDir, "build.gradle.kts"))) return "gradle";
  if (exists(path14.join(rootDir, "composer.lock"))) return "composer";
  if (exists(path14.join(rootDir, "Gemfile.lock"))) return "bundler";
  return "unknown";
}
function detectTestFramework(rootDir) {
  const pkg = readJson2(
    path14.join(rootDir, "package.json")
  );
  if (pkg) {
  ...
  if (exists(path14.join(rootDir, "pytest.ini")) || exists(path14.join(rootDir, "conftest.py"))) return "pytest";
  if (exists(path14.join(rootDir, "phpunit.xml")) || exists(path14.join(rootDir, "phpunit.xml.dist"))) return "PHPUnit";
  if (exists(path14.join(rootDir, "RSpec"))) return "RSpec";
  return void 0;
}
function detectLinter(rootDir) {
  if (exists(path14.join(rootDir, ".eslintrc.json")) || exists(path14.join(rootDir, ".eslintrc.js")) || exists(path14.join(rootDir, ".eslintrc.cjs")) || exists(path14.join(rootDir, "eslint.config.js")) || exists(path14.join(rootDir, "eslint.config.mjs"))) return "ESLint";
  if (exists(path14.join(rootDir, ".biome.json")) || exists(path14.join(rootDir, "biome.json"))) return "Biome";
  if (exists(path14.join(rootDir, ".oxlintrc.json"))) return "oxlint";
  if (exists(path14.join(rootDir, "pylintrc")) || exists(path14.join(rootDir, ".pylintrc"))) return "Pylint";
  if (exists(path14.join(rootDir, ".flake8")) || exists(path14.join(rootDir, "setup.cfg"))) return "Flake8";
  if (exists(path14.join(rootDir, "clippy.toml")) || exists(path14.join(rootDir, ".clippy.toml"))) return "Clippy";
  if (exists(path14.join(rootDir, ".golangci.yml")) || exists(path14.join(rootDir, ".golangci.yaml"))) return "golangci-lint";
  return void 0;
}
function detectFormatter(rootDir) {
  if (exists(path14.join(rootDir, ".prettierrc")) || exists(path14.join(rootDir, ".prettierrc.json")) || exists(path14.join(rootDir, ".prettierrc.js")) || exists(path14.join(rootDir, "prettier.config.js"))) return "Prettier";
  if (exists(path14.join(rootDir, ".biome.json")) || exists(path14.join(rootDir, "biome.json"))) return "Biome";
  if (exists(path14.join(rootDir, ".editorconfig"))) return "EditorConfig";
  if (exists(path14.join(rootDir, ".rustfmt.toml"))) return "rustfmt";
  if (exists(path14.join(rootDir, ".gofmt"))) return "gofmt";
  return void 0;
}
function detectBundler(rootDir) {
  const pkg = readJson2(path14.join(rootDir, "package.json"));
  ...
  if (exists(path14.join(rootDir, "vite.config.ts")) || exists(path14.join(rootDir, "vite.config.js"))) return "Vite";
  if (exists(path14.join(rootDir, "webpack.config.js")) || exists(path14.join(rootDir, "webpack.config.ts"))) return "Webpack";
  return void 0;
}
function detectCiCd(rootDir) {
  if (exists(path14.join(rootDir, ".github", "workflows"))) return { hasCiCd: true, provider: "GitHub Actions" };
  if (exists(path14.join(rootDir, ".gitlab-ci.yml"))) return { hasCiCd: true, provider: "GitLab CI" };
  if (exists(path14.join(rootDir, ".circleci", "config.yml"))) return { hasCiCd: true, provider: "CircleCI" };
  if (exists(path14.join(rootDir, "Jenkinsfile"))) return { hasCiCd: true, provider: "Jenkins" };
  if (exists(path14.join(rootDir, ".travis.yml"))) return { hasCiCd: true, provider: "Travis CI" };
  if (exists(path14.join(rootDir, "azure-pipelines.yml"))) return { hasCiCd: true, provider: "Azure Pipelines" };
  if (exists(path14.join(rootDir, "bitbucket-pipelines.yml"))) return { hasCiCd: true, provider: "Bitbucket Pipelines" };
  return { hasCiCd: false };
}
function detectNamingConvention(rootDir) {
  const srcDir = exists(path14.join(rootDir, "src")) ? path14.join(rootDir, "src") : rootDir;
  try {
...
function detectPatterns(rootDir) {
  ...
  const pkg = readJson2(path14.join(rootDir, "package.json"));
  const hasTypeScript = exists(path14.join(rootDir, "tsconfig.json")) || Object.keys(pkg?.devDependencies ?? {}).includes("typescript");
  const testDirs = ["__tests__", "tests", "test", "spec", "__spec__"];
  const testDirectory = testDirs.find((d) => exists(path14.join(rootDir, d)));
  return {
    ...
    hasTypeScript,
    hasDockerfile: exists(path14.join(rootDir, "Dockerfile")) || exists(path14.join(rootDir, "docker-compose.yml")),
    hasCiCd,
    ciCdProvider,
    monorepo: exists(path14.join(rootDir, "pnpm-workspace.yaml")) || exists(path14.join(rootDir, "lerna.json")) || exists(path14.join(rootDir, "nx.json")) || exists(path14.join(rootDir, "turbo.json")),
    srcDirectory: exists(path14.join(rootDir, "src")),
    testDirectory
  };
}
...
function getProjectName(rootDir) {
  try {
    const pkg = JSON.parse(fs12.readFileSync(path15.join(rootDir, "package.json"), "utf-8"));
    if (pkg.name) return pkg.name.replace(/^@[^/]+\//, "");
  } catch {
  }
  try {
    const goMod = fs12.readFileSync(path15.join(rootDir, "go.mod"), "utf-8");
    const match2 = goMod.match(/^module\s+(\S+)/m);
    if (match2) return match2[1].split("/").pop() ?? path15.basename(rootDir);
  } catch {
  }
  try {
    const cargo = fs12.readFileSync(path15.join(rootDir, "Cargo.toml"), "utf-8");
    const match2 = cargo.match(/^name\s*=\s*"([^"]+)"/m);
...
function getKeyFiles(rootDir) {
  ...
  return candidates.map((c) => path15.join(rootDir, c)).filter((p) => fs12.existsSync(p)).map((p) => path15.relative(rootDir, p));
}
function getAllDependencies(rootDir) {
  ...
  try {
    const req = fs12.readFileSync(path15.join(rootDir, "requirements.txt"), "utf-8");
    req.split("\n").forEach((line) => {
...
function hasManifest(dir) {
  ...
  return manifests.some((manifest) => fs12.existsSync(path15.join(dir, manifest)));
}
...
function addWorkspaceChildren(absBase, packageRoots) {
  ...
  for (const entry of fs12.readdirSync(absBase, { withFileTypes: true })) {
    ...
    const candidate = path15.join(absBase, entry.name);
...
function discoverPackageRoots(rootDir) {
  ...
  try {
    const pnpmWs = fs12.readFileSync(path15.join(rootDir, "pnpm-workspace.yaml"), "utf-8");
    for (const glob2 of parsePnpmWorkspaceYaml(pnpmWs)) {
      const normalized = glob2.replace(/\\/g, "/").replace(/\/\*\*$/, "").replace(/\/\*$/, "");
      const absBase = path15.join(rootDir, normalized);
  ...
  try {
    const lerna = JSON.parse(fs12.readFileSync(path15.join(rootDir, "lerna.json"), "utf-8"));
    for (const glob2 of lerna.packages ?? []) {
      const normalized = glob2.replace(/\\/g, "/").replace(/\/\*\*$/, "").replace(/\/\*$/, "");
      const absBase = path15.join(rootDir, normalized);
      if (hasManifest(absBase)) packageRoots.add(absBase);
      addWorkspaceChildren(absBase, packageRoots);
  ...
  if (fs12.existsSync(path15.join(rootDir, "nx.json"))) {
    for (const rel of ["apps", "libs"]) {
      addWorkspaceChildren(path15.join(rootDir, rel), packageRoots);
    }
  }
  if (fs12.existsSync(path15.join(rootDir, "turbo.json"))) {
...
function mergeLanguages(profiles) {
  ...
  for (const profile of profiles) {
    for (const lang of profile.languages) {
      ...
      for (const ext2 of lang.extensions) existing.extensions.add(ext2);
...
function detectBuildCommands(rootDir) {
  ...
  if (!commands.test || !commands.build) {
    try {
      const toml = fs12.readFileSync(path15.join(rootDir, "pyproject.toml"), "utf-8");
  ...
  try {
    const makefile = fs12.readFileSync(path15.join(rootDir, "Makefile"), "utf-8");
  ...
  if (!commands.build && fs12.existsSync(path15.join(rootDir, "go.mod"))) {
    commands.build = "go build ./...";
    if (!commands.test) commands.test = "go test ./...";
  }
  if (!commands.build && fs12.existsSync(path15.join(rootDir, "Cargo.toml"))) {
    commands.build = "cargo build";
    if (!commands.test) commands.test = "cargo test";
  }
  if (!commands.build && fs12.existsSync(path15.join(rootDir, "pom.xml"))) {
    commands.build = "mvn compile";
    if (!commands.test) commands.test = "mvn test";
  }
  if (!commands.build && (fs12.existsSync(path15.join(rootDir, "build.gradle")) || fs12.existsSync(path15.join(rootDir, "build.gradle.kts")))) {
...
var __dirname3 = path16.dirname(fileURLToPath5(import.meta.url));
var TEMPLATES_DIR = resolveTemplatesDir(__dirname3);
function readTemplate(name) {
  try {
    return fs13.readFileSync(path16.join(TEMPLATES_DIR, name), "utf-8");
  } catch {
    return "";
  }
}
function readFrameworkTemplate(templateKey) {
  return readTemplate(path16.join("frameworks", `${templateKey}.md`));
}
...
function buildSkillRoutingSection(outputDir) {
  const skillsDir = path16.join(outputDir, ".github", "copilot", "skills");
  if (!fs13.existsSync(skillsDir)) return "";
  const rows = [];
  for (const file of fs13.readdirSync(skillsDir)) {
    if (!file.endsWith(".md")) continue;
    try {
      const raw = fs13.readFileSync(path16.join(skillsDir, file), "utf-8");
...
function generatePathSpecificInstructions(stack, githubDir) {
  const files = [];
  const root = path16.dirname(githubDir);
  const instructionsDir = path16.join(githubDir, "instructions");
  ...
  const hasFrontend = frontendPaths.some((p) => fs13.existsSync(path16.join(root, p)));
  if (hasFrontend) {
    const applyPaths = frontendPaths.filter((p2) => fs13.existsSync(path16.join(root, p2)));
    ...
    const p = path16.join(instructionsDir, "frontend.instructions.md");
    writeIfChanged(p, content);
    files.push(p);
  }
  const backendPaths = ["src/api", "server", "routes", "src/routes", "api", "src/server"];
  const hasBackend = backendPaths.some((p) => fs13.existsSync(path16.join(root, p)));
  if (hasBackend) {
    const applyPaths = backendPaths.filter((p2) => fs13.existsSync(path16.join(root, p2)));
    const applyTo = applyPaths.map((p2) => `${p2}/**`).join(", ");
    const content = [
      "---",
      `applyTo: "${applyTo}"`,
    ...
    const p = path16.join(instructionsDir, "backend.instructions.md");
    writeIfChanged(p, content);
    files.push(p);
  }
  const testExts = ["test.ts", "test.tsx", "spec.ts", "spec.tsx", "test.js", "spec.js"];
  const hasTestFiles = testExts.some((ext2) => {
    try {
      const out = fs13.readdirSync(root).some((f) => f.endsWith(`.${ext2}`));
      return out;
    } catch {
      return false;
    }
  });
  const hasTestDir = stack.patterns.testDirectory ? fs13.existsSync(path16.join(root, stack.patterns.testDirectory)) : false;
  if (hasTestDir || stack.patterns.testFramework) {
    ...
    const p = path16.join(instructionsDir, "tests.instructions.md");
    writeIfChanged(p, content);
    files.push(p);
  }
  const schemaPaths = ["prisma", "migrations", "db/migrations", "src/db"];
  const hasSchema = schemaPaths.some((p) => fs13.existsSync(path16.join(root, p)));
  if (hasSchema || stack.allDependencies.includes("prisma") || stack.allDependencies.includes("@prisma/client")) {
    const applyPaths = schemaPaths.filter((p2) => fs13.existsSync(path16.join(root, p2)));
    ...
    const p = path16.join(instructionsDir, "schema.instructions.md");
    writeIfChanged(p, content);
    files.push(p);
  }
  return files;
...
function buildPersistentRulesSection(persistentRules, stack) {
  ...
  for (const up of utilsPaths) {
    if (fs13.existsSync(path16.join(root, up))) {
  ...
  for (const ap of apiPaths) {
    if (fs13.existsSync(path16.join(root, ap))) {
  ...
  for (const tp of typePaths) {
    if (fs13.existsSync(path16.join(root, tp))) {
...
No specific framework template found. Follow the general rules above.`, outputDir);
  ...
  const githubDir = path16.join(outputDir, ".github");
  const outputPath = path16.join(githubDir, "copilot-instructions.md");
  ...
  const autoActivationPath = path16.join(instructionsDir, "ai-os.instructions.md");
...
function generatePromptQualityPack(stack, outputDir, githubDir) {
  const agentsDir = path16.join(outputDir, ".github", "agents");
  const skillsDir = path16.join(outputDir, ".github", "copilot", "skills");
  const agentRows = [];
  if (fs13.existsSync(agentsDir)) {
    for (const file of fs13.readdirSync(agentsDir)) {
      ...
      try {
        const raw = fs13.readFileSync(path16.join(agentsDir, file), "utf-8");
        const nameMatch = raw.match(/^name:\s*(.+)$/m);
  ...
  const outputPath = path16.join(instructionsDir, "prompt-quality.instructions.md");
  writeIfChanged(outputPath, content);
  return outputPath;
}
...
function writeCopilotCliMcpConfig(outputDir, options) {
  const mcpJsonPath = path17.join(outputDir, ".mcp.json");
...
function writeVsCodeMcpConfig(outputDir, options) {
  const mcpJsonPath = path17.join(outputDir, ".vscode", "mcp.json");
  const existing = readJsonObject(mcpJsonPath);
...
function generateMcpJson(stack, outputDir, options) {
  ...
  const toolsJsonPath = path17.join(outputDir, ".github", "ai-os", "tools.json");
  if (strictFiltering) {
...
function writeMcpServerConfig(outputDir, options) {
  ...
  return path17.join(outputDir, ".vscode", "mcp.json");
}
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (SOURCE_EXTENSIONS.has(ext2)) {
          files.push(path18.relative(rootDir, full).replace(/\\/g, "/"));
        }
      }
    }
  } catch {
...
function parseImports(content, filePath) {
  ...
  if (["ts", "tsx", "js", "jsx", "mjs", "cjs"].includes(ext2)) {
    ...
    while ((m = importRe.exec(content)) !== null) {
      ...
      if (spec.startsWith(".")) {
        ...
        const resolved = path18.posix.join(dir, spec);
...
function parseExports(content, ext2) {
  const exports = [];
  if (["ts", "tsx", "js", "jsx", "mjs", "cjs"].includes(ext2)) {
...
function resolveImportPath(importSpec, allFiles) {
  ...
  for (const ext2 of exts) {
    const candidate = `${importSpec}.${ext2}`;
    if (allFiles.includes(candidate)) return candidate;
  }
  for (const ext2 of exts) {
    const candidate = `${importSpec}/index.${ext2}`;
    if (allFiles.includes(candidate)) return candidate;
  }
  if (importSpec.endsWith(".js")) {
    const base = importSpec.slice(0, -3);
    for (const ext2 of ["ts", "tsx"]) {
...
function buildDependencyGraph(rootDir) {
  ...
  for (const file of allFiles) {
    try {
      const content = fs15.readFileSync(path18.join(rootDir, file), "utf-8");
      const ext2 = file.split(".").pop()?.toLowerCase() ?? "";
      nodes[file].exports = parseExports(content, ext2);
...
function readAiOsConfig(outputDir) {
  const configPath = path19.join(outputDir, ".github", "ai-os", "config.json");
  try {
...
function exists2(root, relativePath) {
  return fs16.existsSync(path19.join(root, relativePath));
}
...
function generateArchitectureDoc(stack) {
  ...
  try {
    ...
    for (const entry of entries.slice(0, 20)) {
      const stat = fs16.statSync(path19.join(stack.rootDir, entry));
...
function generateContextDocs(stack, outputDir, options) {
  ...
  const contextDir = path19.join(outputDir, ".github", "ai-os", "context");
  fs16.mkdirSync(contextDir, { recursive: true });
  const memoryDir = path19.join(outputDir, ".github", "ai-os", "memory");
  ...
  const legacyMemory = path19.join(outputDir, ".ai-os", "memory", "memory.jsonl");
  const newMemory = path19.join(memoryDir, "memory.jsonl");
  if (fs16.existsSync(legacyMemory) && !fs16.existsSync(newMemory)) {
    fs16.copyFileSync(legacyMemory, newMemory);
  }
  const stackPath = track(path19.join(contextDir, "stack.md"));
  if (!shouldPreserve(stackPath)) {
    writeIfChanged(stackPath, generateStackDoc(stack));
  }
  const archPath = track(path19.join(contextDir, "architecture.md"));
  ...
  const convsPath = track(path19.join(contextDir, "conventions.md"));
  ...
  writeIfChanged(track(path19.join(contextDir, "memory.md")), generateMemoryDoc(stack));
  const existingAiContextPath = track(path19.join(contextDir, "existing-ai-context.md"));
  ...
  const contextBudgetPath = track(path19.join(contextDir, "context-budget.md"));
  if (!shouldPreserve(contextBudgetPath)) {
    writeIfChanged(contextBudgetPath, generateContextBudgetDoc(stack));
  }
  const protectedBlocksPath = track(path19.join(contextDir, "protected-blocks.md"));
  if (!shouldPreserve(protectedBlocksPath)) {
    writeIfChanged(protectedBlocksPath, generateProtectedBlocksDoc());
  }
  const memoryReadmePath = track(path19.join(memoryDir, "README.md"));
  ...
  const memoryFilePath = track(path19.join(memoryDir, "memory.jsonl"));
  ...
  writeIfChanged(track(path19.join(contextDir, "dependency-graph.json")), JSON.stringify(graph, null, 2));
  const toolRefPath = track(path19.join(contextDir, "mcp-tools.md"));
  ...
  const aiOsDir = path19.join(outputDir, ".github", "ai-os");
  writeIfChanged(track(path19.join(aiOsDir, "config.json")), JSON.stringify(config, null, 2));
  if (config.sessionContextCard) {
    const sessionCardPath = track(path19.join(outputDir, ".github", "COPILOT_CONTEXT.md"));
...
function generateSessionContextCard(stack, config, outputDir) {
  ...
  const skillsCount = (() => {
    ...
    const skillsDir = path19.join(outputDir, ".github", "copilot", "skills");
...
function discoverProjectKeyFiles(cwd, stack, limit = 6) {
  const exists4 = (rel) => fs17.existsSync(path20.join(cwd, rel));
  const found = [];
  const wellKnown = [
    "prisma/schema.prisma",
    "src/app/layout.tsx",
...
function buildAgentSpecs(stack, cwd) {
  const specs = [];
  const projectName = sanitizeForInstructions(path20.basename(cwd));
  ...
  const runtimeDir = path20.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Z]:)/, "$1"));
  const templateDir = path20.join(resolveTemplatesDir(runtimeDir), "agents");
  specs.push({
    templateFile: path20.join(templateDir, "repo-initializer.md"),
  ...
  specs.push({
    templateFile: path20.join(templateDir, "framework-expert.md"),
  ...
  specs.push({
    templateFile: path20.join(templateDir, "codebase-explorer.md"),
  ...
  if (hasPrisma) {
    const schemaFile = fs17.existsSync(path20.join(cwd, "prisma/schema.prisma")) ? "prisma/schema.prisma" : "schema.prisma";
    specs.push({
      templateFile: path20.join(templateDir, "db-expert.md"),
  ...
  if (hasAuth) {
    ...
    specs.push({
      templateFile: path20.join(templateDir, "auth-expert.md"),
  ...
  if (hasStripe) {
    const plansFile = fs17.existsSync(path20.join(cwd, "src/constants/stripe.ts")) ? "src/constants/stripe.ts" : "src/lib/stripe.ts";
    specs.push({
      templateFile: path20.join(templateDir, "payments-expert.md"),
  ...
  specs.push({
    templateFile: path20.join(templateDir, "architecture-migration.md"),
...
function scanExistingAgents(cwd) {
  const agentsDir = path20.join(cwd, AGENTS_DIR);
  ...
  const aiOsGenerated = [];
  for (const file of files) {
    const content = fs17.readFileSync(path20.join(agentsDir, file), "utf-8");
...
function buildSequentialAgentSpecs(stack, cwd) {
  ...
  specs.push({
    templateFile: path20.join(templateDir, "enhancement-advisor.md"),
  ...
  specs.push({
    templateFile: path20.join(templateDir, "idea-validator.md"),
  ...
  specs.push({
    templateFile: path20.join(templateDir, "implementation-agent.md"),
...
async function generateAgentsWithOptions(stack, cwd, options) {
  ...
  for (const spec of specs) {
    const outputPath = path20.join(agentsDir, spec.outputFile);
...
function buildSkillSpecs(stack, cwd) {
  const specs = [];
  const projectName = path21.basename(cwd);
  const frameworks = stack.frameworks.map((f) => f.name.toLowerCase());
  const packages = stack.allDependencies;
  ...
  const templateDir = path21.join(resolveTemplatesDir(path21.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Z]:)/, "$1"))), "skills");
  const add = (template, output, replacements = {}) => {
    const templatePath = path21.join(templateDir, template);
  ...
  if (packages.includes("@trpc/server") || packages.includes("trpc")) {
    const trpcRouterFile = fs18.existsSync(path21.join(cwd, "src/trpc/index.ts")) ? "src/trpc/index.ts" : "src/server/trpc.ts";
  ...
  if (packages.includes("prisma") || packages.includes("@prisma/client")) {
    const schemaFile = fs18.existsSync(path21.join(cwd, "prisma/schema.prisma")) ? "prisma/schema.prisma" : "schema.prisma";
    add("prisma.md", "ai-os-prisma-patterns.md", { "{{SCHEMA_FILE}}": schemaFile });
  }
  if (packages.includes("stripe")) {
    const plansFile = fs18.existsSync(path21.join(cwd, "src/constants/stripe.ts")) ? "src/constants/stripe.ts" : "src/lib/stripe.ts";
    add("stripe.md", "ai-os-billing-stripe.md", {
      "{{PLANS_FILE}}": plansFile,
      "{{STRIPE_LIB_FILE}}": fs18.existsSync(path21.join(cwd, "src/lib/stripe.ts")) ? "src/lib/stripe.ts" : plansFile,
      "{{WEBHOOK_FILE}}": "src/app/api/webhooks/stripe/route.ts"
    });
  }
  if (packages.includes("next-auth") || packages.includes("nextauth")) {
    const authFile = fs18.existsSync(path21.join(cwd, "src/app/api/auth/[...nextauth]/authOptions.ts")) ? "src/app/api/auth/[...nextauth]/authOptions.ts" : "src/lib/auth.ts";
...
async function generateSkillsWithOptions(stack, cwd, options) {
  const skillsDir = path21.join(cwd, SKILLS_DIR);
  ...
  if (options.strategy === "creator-only") {
    if (options.refreshExisting && fs18.existsSync(skillsDir)) {
      ...
      for (const stale of onDisk) {
        fs18.rmSync(path21.join(skillsDir, stale));
  ...
  for (const spec of specs) {
    const outputPath = path21.join(skillsDir, spec.outputFile);
  ...
  if (options.refreshExisting && fs18.existsSync(skillsDir)) {
    const currentSet = new Set(generatedPaths.map((p) => path21.basename(p)));
    const onDisk = fs18.readdirSync(skillsDir).filter((f) => f.startsWith("ai-os-") && f.endsWith(".md"));
    for (const stale of onDisk) {
...
async function deployBundledSkills(cwd, options) {
  ...
  for (const skill of BUNDLED_SKILLS) {
    ...
    const targetDir = path21.join(cwd, AGENTS_SKILLS_DIR, skill.dirName);
    ...
    fs18.mkdirSync(path21.join(cwd, AGENTS_SKILLS_DIR), { recursive: true });
...
function generateWorkflows(outputDir, options) {
  ...
  if (options?.config?.updateCheckEnabled !== false) {
    const workflowPath = track(path23.join(outputDir, ".github", "workflows", "ai-os-update-check.yml"));
...
function generateToolsets(stack, outputDir) {
  ...
  const toolsetsPath = path24.join(outputDir, ".vscode", "toolsets.json");
...
function generateChatModes(stack, outputDir) {
  const managed = [];
  const vscodePath = path25.join(outputDir, ".vscode");
  const modes = [
    getPlanMode(stack),
    getReviewMode(stack),
    getExploreMode()
  ...
  for (const mode of modes) {
    const filePath = path25.join(vscodePath, mode.filename);
...
function exists3(root, relPath) {
  return fs20.existsSync(path26.join(root, relPath));
}
...
function collectRecommendations(stack) {
  ...
  function applyRec(rec, isUniversal = false) {
    ...
    for (const ext2 of rec.vscode ?? []) {
      if (!seenVscode.has(ext2)) {
        seenVscode.add(ext2);
        collected.vscode.push({ trigger: rec.trigger, id: ext2 });
      }
    }
...
function generateRecommendations(stack, outputDir) {
  ...
  const outPath = path27.join(outputDir, ".github", "ai-os", "recommendations.md");
...
function extractUserBlocks(content) {
  ...
  let match2;
  while ((match2 = BLOCK_GLOBAL_RE.exec(content)) !== null) {
    const id = match2[1];
    const innerContent = match2[2];
    const fullMatch = match2[0];
    if (blocks.has(id)) continue;
    const beforeContent = content.slice(0, match2.index);
...
function runBootstrap(stack, options = {}) {
  ...
  for (const ext2 of recs.vscode) {
    items.push({
      category: "vscode",
      name: ext2.id,
      reason: `triggered by: ${ext2.trigger}`,
      installCmd: `code --install-extension ${ext2.id}`,
      status: dryRun ? "pending" : "skipped"
      // Must be installed manually
    });
  }
  for (const ext2 of recs.copilotExtensions) {
    items.push({
...
function ensureGitignoreEntry(cwd, entry) {
  const gitignorePath = path28.join(cwd, ".gitignore");
...
function resolveBundledServerSource() {
  const runtimeDir = path28.dirname(fileURLToPath6(import.meta.url));
  const candidates = [
    path28.join(runtimeDir, "server.js"),
    path28.join(runtimeDir, "..", "bundle", "server.js"),
    path28.join(runtimeDir, "..", "dist", "server.js")
  ];
  for (const candidate of candidates) {
...
function installLocalMcpRuntime(cwd, verbose) {
  ...
  const runtimeDir = path28.join(cwd, ".ai-os", "mcp-server");
  const runtimeEntry = path28.join(runtimeDir, "index.js");
  const runtimeManifest = path28.join(runtimeDir, "runtime-manifest.json");
  const nodePath = process.execPath;
  ...
  const legacyLocalMcp = path28.join(cwd, ".github", "copilot", "mcp.local.json");
...
function printDryRunDiff(cwd, captures, fullDiff) {
  ...
  for (const cap of captures) {
    const rel = path28.relative(cwd, cap.filePath).replace(/\\/g, "/");
...
function printSummary(stack, outputDir, written, skipped, pruned, agents, preserved, activeProfile) {
  ...
  if (preserved.length > 0) {
    ...
    for (const p of preserved) console.log(`       \u2022 ${path28.relative(outputDir, p).replace(/\\/g, "/")}`);
  }
  if (pruned.length > 0) {
    console.log(`  \u{1F5D1}\uFE0F  Pruned (stale):              ${pruned.length}`);
    for (const p of pruned) console.log(`       \u2022 ${path28.relative(outputDir, p).replace(/\\/g, "/")}`);
  }
  if (agents.length > 0) {
    console.log(`  \u{1F916} Agents generated: ${agents.length}`);
  }
  console.log(`  \u{1F527} MCP tools registered: ${mcpToolCount}`);
  console.log(`  \u{1F5F3}\uFE0F  Manifest: ${path28.relative(outputDir, getManifestPath(outputDir)).replace(/\\/g, "/")}`);
  try {
...
function printMemoryMaintenanceSummary(cwd) {
  const memoryFile = path28.join(cwd, ".github", "ai-os", "memory", "memory.jsonl");
...
function validateSkillRoutingCompleteness(cwd) {
  const skillsDir = path28.join(cwd, ".github", "copilot", "skills");
  ...
  try {
    for (const file of fs22.readdirSync(skillsDir)) {
      if (!file.endsWith(".md")) continue;
      try {
        const raw = fs22.readFileSync(path28.join(skillsDir, file), "utf-8");
...
async function runApply(args) {
  ...
  \u{1F39B}\uFE0F  Applying profile: ${cliProfile}`);
    ...
    if (config) {
      ...
      if (!dryRun) {
        const configPath = path28.join(cwd, ".github", "ai-os", "config.json");
  ...
  if (config?.recommendations !== false) {
    ...
    const skillsLockPath = path28.join(path28.dirname(new URL(import.meta.url).pathname), "..", "skills-lock.json");
...
`);
  ...
  const toRel = (p) => path28.relative(cwd, p).replace(/\\/g, "/");
    ...
    for (const rel of previousFiles) {
      if (!currentSet.has(rel)) {
        if (protectedPaths.has(rel)) {
          ...
          preservedAbs.push(path28.join(cwd, rel));
          continue;
        }
  ...
  if (!dryRun) {
    for (const [abs, originalContent] of protectedSnapshots) {
      ...
      if (currentContent !== originalContent) {
        ...
        const rel = path28.relative(cwd, abs).replace(/\\/g, "/");
  ...
  if (quiet) {
    ...
    if (action === "bootstrap") {
      ...
      console.log(JSON.stringify({
        ...
        pruned: prunedAbs.map((p) => path28.relative(cwd, p).replace(/\\/g, "/")),
        agents: agentFiles,
        preserved: preservedAbs.map((p) => path28.relative(cwd, p).replace(/\\/g, "/")),
  ...
  if (!dryRun && isFirstInstall) {
    const spLockPath = path28.join(path28.dirname(new URL(import.meta.url).pathname), "..", "skills-lock.json");
...
function readProtectedPaths(cwd) {
  ...
  try {
    ...
    return new Set(files.map((f) => path29.resolve(cwd, f)));
  } catch {
...
function runUninstall(cwd, options = {}) {
  ...
  for (const relPath of manifest.files) {
    const abs = path29.resolve(cwd, relPath);
    ...
    try {
      fs23.unlinkSync(abs);
      report.removed.push(relPath);
      affectedDirs.add(path29.dirname(abs));
  ...
  const managedDirs = [
    path29.join(cwd, ".ai-os", "mcp-server"),
    path29.join(cwd, ".ai-os")
  ];
  const manifestPath = path29.join(cwd, ".github", "ai-os", "manifest.json");
  if (!dryRun) {
    for (const dir of managedDirs) {
      try {
        if (fs23.existsSync(dir)) {
          fs23.rmSync(dir, { recursive: true, force: true });
          if (verbose) console.log(`  \u{1F5D1}\uFE0F  removed    ${path29.relative(cwd, dir)}/`);
        }
      } catch (err) {
        const reason = err instanceof Error ? err.message : String(err);
        console.error(`  \u2716 error       ${path29.relative(cwd, dir)}: ${reason}`);
      }
    }
    try {
    ...
    const dirsToCheck = [
      ...
      path29.join(cwd, ".github", "ai-os"),
      path29.join(cwd, ".github", "agents"),
      path29.join(cwd, ".github", "copilot", "skills"),
      path29.join(cwd, ".github", "copilot"),
      path29.join(cwd, ".github", "instructions")
    ];
