...
import {
  getAllMcpTools as getRuntimeTools,
  getActiveToolsForProject,
} from '../mcp-server/tool-definitions.js';
...
describe('getActiveToolsForProject', () => {
  ...
  it('reads activeTools from new-format tools.json', () => {
    const tmpDir = path.join(
      os.tmpdir(),
      `mcp-test-new-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('reads all tools from legacy flat-array tools.json', () => {
    const tmpDir = path.join(
      os.tmpdir(),
      `mcp-test-legacy-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    const aiOsDir = path.join(tmpDir, '.github', 'ai-os');
    fs.mkdirSync(aiOsDir, { recursive: true });
